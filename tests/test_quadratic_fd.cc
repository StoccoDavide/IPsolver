...
public:
  ...
  {
    IPsolver::FiniteDifferences::Gradient(
      x, [this] (const VectorN & fd_x, Real & fd_out) {return this->objective(fd_x, fd_out);}, out
    );
  ...
  {
    IPsolver::FiniteDifferences::Hessian(
      x, [this] (const VectorN & fd_x, Real & fd_out) {return this->objective(fd_x, fd_out);}, out
    );
    return out.allFinite();
  }
  ...
  {
    IPsolver::FiniteDifferences::Jacobian(
      x, [this] (const VectorN & fd_x, VectorM & fd_out) {return this->constraints(fd_x, fd_out);}, out
    );
    return out.allFinite();
  }
  ...
  {
    IPsolver::FiniteDifferences::Hessian(
      x, [this, &z] (const VectorN & fd_x, Real & fd_out) {
        Real obj_val;
        this->objective(fd_x, obj_val);
        VectorM constr_val;
        this->constraints(fd_x, constr_val);
        fd_out = obj_val;
        for (Integer i{0}; i < static_cast<Integer>(z.size()); ++i) {
          fd_out += z(i) * constr_val(i);
        }
        return std::isfinite(fd_out);
      },
      out
    );
    return out.allFinite();
